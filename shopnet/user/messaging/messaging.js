		...
		fetch("fetch_insert_msg.php", {
			...
			.then(data => {
				if (data == 1) {
					...
					{
						newPartner  = `<li class="seller active" id="${idPartner}" onclick="accessChat(event)">
...
{
	idPartner = location.search.slice(4)
	var e = document.getElementById(idPartner)
	if(e)
	  e.click()
	else{
		fetch(`fetch_seller.php?id=${idPartner}`)
		.then(res=>res.json())
		.then(data=>{
			chatHeaderTitle.innerText = data.username
			chatHeaderImg.src = `../../upload/${data.image}`
		})
		.catch(err=>{
			console.log(err)
		})
		...
		.then(data => {
			listPartners.innerHTML = '';
				for (i = 0; i < data.length; i++) {
					listPartners.innerHTML +=  `<li class="seller" id="${data[i].seller_id}" onclick="accessChat(event)">
					                              <img src="../../upload/${data[i].image}">
