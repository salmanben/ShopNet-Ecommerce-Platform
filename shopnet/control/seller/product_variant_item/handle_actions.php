...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_GET['action'])) {
    if ($_GET['action'] == 'create'){
        ...
        $name = ucfirst($name);
        $sql = "INSERT INTO product_variant_items (name, variant_id, status, is_default, price)
               VALUES ('$name', '{$_POST['variant_id']}', $status, $is_default, $price)";
        $result = mysqli_query($connect, $sql);
    
        if ($result) {
            echo json_encode(['status' => 'success', 'message' => "Product variant item created successfully."]);
        } else {
        ...
        if ($_POST["is_default"] == 1 && $_POST["price"] != 0) {
            echo json_encode(['status' => 'error', 'message' => "The default variant item must have a price equal to 0."]);
            exit();
        }
        if ($is_default == 1 && $price!= 0) {
