...
if (!isset($_GET['product_id']) || !isset($_GET['product_variant_id']))
{
    http_response_code(404);
    exit; 
}
...
$sql = "SELECT * FROM product_variants WHERE id = '{$_GET['product_variant_id']}' 
...
<head>
    ...
    <title>Product Variants | Update</title>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form onsubmit="updateItem(event)" action="handle_actions.php" method="post" enctype="multipart/form-data">
                            ...
                            <div>
                                <input type="hidden" name="product_variant_id" value="<?php echo $_GET['product_variant_id']; ?>">
                            </div>
                            <div>
                                <label for="name">Name:</label>
                                <input class="title" value="<?php echo $product_variant['name'] ?>" type="text" name="name" placeholder="Name" id="name">
                            </div>
                            <div>
                                <label for="status">Status:</label>
                                <select name="status" class="type" id="status">
                                    <option <?php if ($product_variant['status'] == 1) echo "selected" ?> value="1">Active</option>
                                    <option <?php if ($product_variant['status'] == 0) echo "selected" ?> value="0">Inactive</option>
                                </select>
                            </div>
                            <div>
